                ...
                waitFor.nextTick("request", async () => {
                    resolve(
                        await this.page.evaluate(() => {
                            return Array.from(
                                document.querySelectorAll("li[id*=course]")
                            )
                                .filter(
                                    (el) => !el.querySelector(".not-open-tip")
                                )
                                .map((el: any) => ({
                                    img: el.querySelector(
                                        ".course-cover > a > img"
                                    ).src,
                                    url: el.querySelector(".course-cover > a")
                                        .href,
                                    profile:
                                        el.querySelector(".course-info")
                                            .innerText,
                                }));
                        })
                    );
