	...
	>
		...
		<template v-if="hide">
			<Tooltip
				...
				:title="`OCS 网课助手 ${useStore(
					'VERSION'
				)}<br>单击拖动<br>双击展开<br>连续按下ocs三个键可复原位置<br>想要完全隐藏可移出屏幕`"
			>
				<img
					ref="logo"
					class="ocs-icon"
 ...
 */
const activeKey = ref(
	currentPanels.value.find((p) => p.name === local.activeKey)?.name || currentPanels.value[0]?.name
);
/**
 * 元素
 */
